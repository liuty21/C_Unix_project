...
//=======================  ResNet  ========================//
//=========================================================//
ResNet::ResNet(int dim1, int dim2, int dim3, int kernel_size1, 
    int stride1, int stride2, int stride3, int padding1, 
    int fc_size1, int fc_size2, int fc_size3):
_dim1(dim1), _dim2(dim2), _dim3(dim3), _kernel_size1(kernel_size1),
_stride1(stride1), _stride2(stride2), _stride3(stride3), _padding1(padding1),
_fc_size1(fc_size1), _fc_size2(fc_size2), _fc_size3(fc_size3),
conv(1, dim1, kernel_size1, stride1, padding1), pool(2,2), 
...
int ResNet::set_weight(char* model_path)
{
    // -------- open model file -------- //
    FILE* fi = fopen(model_path, "r");
    if (fi<0)
    ...
    conv.set_weight(kernel, bias); // set layer weight
    ...
    printf("conv layer initialized\n");

    float** kernel2, ** kernel_i;
    int kernel_len2, kernel_len_i;
    // -------- set residual1 layer parameters -------- //
    // malloc memory
    // res.conv1 weight
    kernel = (float**)malloc(sizeof(float*)*_dim2);
    kernel_len = _dim1 * 3 * 3;
    if (kernel == NULL)
    {
        printf("Malloc failed!\n");
        exit(1);
    ...
    // res.conv2 weight
    kernel2 = (float**)malloc(sizeof(float*)*_dim2);
    kernel_len2 = _dim2 * 3 * 3;
    if (kernel2 == NULL)
    {
        printf("Malloc failed!\n");
        exit(1);
    ...
    for (int i = 0; i < _dim2; ++i)
    {
        kernel2[i] = (float*)malloc(sizeof(float)*kernel_len2);
        if (kernel2[i] == NULL)
        {
            printf("Malloc failed!\n");
            exit(1);
    ...
    // res.conv_i weight
    kernel_i = (float**)malloc(sizeof(float*)*_dim2);
    kernel_len_i = _dim1 * 1 * 1;
    if (kernel_i == NULL)
    {
        printf("Malloc failed!\n");
        exit(1);
    ...
    for (int i = 0; i < _dim2; ++i)
    {
        kernel_i[i] = (float*)malloc(sizeof(float)*kernel_len_i);
        if (kernel_i[i] == NULL)
        {
            printf("Malloc failed!\n");
            exit(1);
    ...
    for (int i = 0; i < _dim2; ++i)
    {
        for (int j = 0; j < kernel_len; ++j)
        {
            fscanf(fi, "%f", &kernel[i][j]); // read in conv1 weight values
        }
    }
    for (int i = 0; i < _dim2; ++i)
    {
        for (int j = 0; j < kernel_len2; ++j)
        {
            fscanf(fi, "%f", &kernel2[i][j]); // read in conv2 weight values
        }
    }
    for (int i = 0; i < _dim2; ++i)
    {
        for (int j = 0; j < kernel_len_i; ++j)
        {
            fscanf(fi, "%f", &kernel_i[i][j]); // read in conv_i weight values
        }
    }
    for (int i = 0; i < _dim2; ++i)
    {
        bias[i] = 0; // bias values are all = 0 by default
    }

    res1.set_weight(kernel, bias, kernel2, bias, kernel_i, bias);
    ...
    printf("res1 layer initialized\n");

    // -------- set residual2 layer parameters -------- //
    // malloc memory
    // res.conv1 weight
    kernel = (float**)malloc(sizeof(float*)*_dim3);
    kernel_len = _dim2 * 3 * 3;
    if (kernel == NULL)
    {
        printf("Malloc failed!\n");
        exit(1);
    ...
    // res.conv2 weight
    kernel2 = (float**)malloc(sizeof(float*)*_dim3);
    kernel_len2 = _dim3 * 3 * 3;
    if (kernel2 == NULL)
    {
        printf("Malloc failed!\n");
        exit(1);
    ...
    // res.conv_i weight
    kernel_i = (float**)malloc(sizeof(float*)*_dim3);
    kernel_len_i = _dim2 * 1 * 1;
    if (kernel_i == NULL)
    {
        printf("Malloc failed!\n");
        exit(1);
    ...
    res2.set_weight(kernel, bias, kernel2, bias, kernel_i, bias);
...
float* ResNet::forward(float** input, float* output, int in_size)
{
    // -------- calculate intermediate outputs size --------//
    int c1_out_size = output_size_cal(in_size, _kernel_size1, _stride1, _padding1); // conv1 output
    int p1_out_size = output_size_cal(c1_out_size, 2, 2); // pooling1 output
    int r1_out_size = output_size_cal(p1_out_size, 3, _stride2, 1); // residual1 output
    int r2_out_size = output_size_cal(r1_out_size, 3, _stride3, 1); // residual2 output
    ...
    // forward calculation
    conv.forward(input, in_size, inner_output);
    ReLU(inner_output, _dim1, c1_out_size);
    ...
    // forward calculation
    pool.forward(inner_input, _dim1, c1_out_size, inner_output);
    ...
    for (int i = 0; i < _dim2; ++i)
    {
        inner_output[i] = (float*)malloc(sizeof(float)*r1_out_size*r1_out_size);
        if (inner_output[i] == NULL)
        {
            printf("Malloc failed!\n");
            exit(1);
    ...
    // forward calculation
    res1.forward(inner_input, p1_out_size, inner_output);
    ...
    for (int i = 0; i < _dim3; ++i)
    {
        inner_output[i] = (float*)malloc(sizeof(float)*r2_out_size*r2_out_size);
        if (inner_output[i] == NULL)
        {
            printf("Malloc failed!\n");
            exit(1);
    ...
    // forward calculation
    res2.forward(inner_input, r1_out_size, inner_output);
    ...
    float* fc_output;// 1d intermediate output pointer
    fc_input = (float*)malloc(sizeof(float)*_fc_size1); 
    // assert _fc_size1 == _dim3*r2_out_size*r2_out_size
    int index = 0;
    for (int d = 0; d < _dim3; ++d)
    {
        for (int i = 0; i < r2_out_size; ++i)
        {
            for (int j = 0; j < r2_out_size; ++j)
            {
                fc_input[index] = inner_output[d][i*r2_out_size+j];
                ++index;
            }
        }
    }
...
//======================  VGGNet  ========================//
//========================================================//
VGGNet::VGGNet(int dim1, int dim2, int dim3,
        int fc_size1, int fc_size2, int fc_size3):
_dim1(dim1), _dim2(dim2), _dim3(dim3),
_fc_size1(fc_size1), _fc_size2(fc_size2), _fc_size3(fc_size3),
conv1(1, dim1, 3, 1, 1),
conv2(dim1, dim2, 3, 1, 1), 
conv3(dim2, dim3, 3, 1, 1),
pool(2,2),
fc1(fc_size1, fc_size2), fc2(fc_size2, fc_size3)
{
...
int VGGNet::set_weight(char* model_path)
{
    // -------- open model file -------- //
    FILE* fi = fopen(model_path, "r");
    if (fi<0)
...
float* VGGNet::forward(float** input, float* output, int in_size)
{
    // -------- calculate intermediate outputs size --------//
    int c1_out_size = output_size_cal(in_size, 3, 1, 1); // conv1 output
    int p1_out_size = output_size_cal(c1_out_size, 2, 2); // pooling1 output
    int c2_out_size = output_size_cal(p1_out_size, 3, 1, 1); // conv2 output
    int p2_out_size = output_size_cal(c2_out_size, 2, 2); // pooling2 output
    int c3_out_size = output_size_cal(p2_out_size, 3, 1, 1); // conv3 output
    int p3_out_size = output_size_cal(c3_out_size, 2, 2); // pooling3 output
    ...
    // forward calculation
    pool.forward(inner_input, _dim2, c2_out_size, inner_output);
    ...
    for (int i = 0; i < _dim3; ++i)
    {
        inner_output[i] = (float*)malloc(sizeof(float)*p3_out_size*p3_out_size);
        if (inner_output[i] == NULL)
        {
            printf("Malloc failed!\n");
            exit(1);
    ...
    // forward calculation
    pool.forward(inner_input, _dim3, c3_out_size, inner_output);
    ...
    float* fc_output;// 1d intermediate output pointer
    fc_input = (float*)malloc(sizeof(float)*_fc_size1); 
    // assert _fc_size1 == _dim3*p3_out_size*p3_out_size
    int index = 0;
    for (int d = 0; d < _dim3; ++d)
    {
        for (int i = 0; i < p3_out_size; ++i)
        {
            for (int j = 0; j < p3_out_size; ++j)
            {
                fc_input[index] = inner_output[d][i*p3_out_size+j];
                ++index;
            }
        }
    }
